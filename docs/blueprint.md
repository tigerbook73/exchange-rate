...
4. 检测本地历史是否有日期空档，分两种情况区别处理：
   ...
   - **28 天源站窗口内的空档（比如周日）→ 用最近一天的真实数据填补，并在展示时标注**：取本地已有的、日期上最近的前一个真实数据（沿用其 `huiSell`），写回 IndexedDB 时标记为填补记录（`source: "carried-forward"`，并记录 `carriedFromDate` 指向真实数据的日期），不能让用户误以为是当天的真实报价。趋势图上这类点用空心圆点（区别于实心点）+ 点击/悬停 tooltip 里的说明文字（"该日期未更新，沿用前一日数据"）呈现——**不要**在图表下方常驻一行说明文字：这类文字是否出现取决于当前数据里是否存在填补点，属于条件渲染，会在真实数据到达的那一刻让图表容器的 flex 兄弟节点突然多出/减少一块空间，进而触发 Chart.js 的 `ResizeObserver` 重新计算画布尺寸，容易引出布局抖动/意外撑高的问题（真实踩过）。**回溯范围限制在 28 天源站窗口内**，不无限回溯。
