...
4. 检测本地历史是否有日期空档：
   - **不假设固定的休市规律，以源站实际返回的日期集合为准**：已用真实数据核对，工行发布节奏是"周六有报价、只有周日没有"，并非"仅工作日"这种直觉假设。判空档时只对比「本地 IndexedDB 已有日期」与「`/api/history` 这次实际返回的日期集合」的差集——源站窗口内本来就没有的日期（不管是不是周日），不算客户端侧的异常空档，也不需要伪造/复用相邻日期的 `huiSell`；这类日期在图表上直接没有数据点，不是断线。
   - 真正的空档指：源站窗口内存在该日期的数据，但本地 IndexedDB 还没有 → 这次拉取的 `/api/history` 会自动补上，无需额外处理。
   - 空档超过 28 天（且确认源站早期窗口也曾有过这天的数据）→ 标记为"数据缺失（源站未提供更早查询）"，在图表上以断线或灰色提示呈现，不做插值伪造数据。
