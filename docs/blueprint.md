...
1. App 启动（或用户下拉刷新）时：
   ...
   - 调用 `/api/history`：先读本地 IndexedDB 里已有数据的最新日期，若存在，带上 `datefrom=<最新日期+1天>` 只请求增量部分；本地为空（首次访问）时不传参，服务端 fallback 到"往前回溯 1 年"，一次性拿到完整历史。
