...
- **空档判断以源站实际发布的日期集合为准，不要假设固定的休市规律**：已用真实数据核对 `d-icbc-aud.html`（2026-06-23 ~ 2026-07-24 期间），**每个周六都有报价，只有周日没有**——工行网站的发布节奏并非"仅工作日"，早期方案里"周六周日都不发布"的假设是错的，不要照搬。正确做法是：只把「本地已有日期」与「`/api/history` 这次实际返回的日期集合」做差集，凡是源站窗口内本来就没有的日期，一律不算作客户端的异常空档，也不需要为它伪造/复用相邻日期的 `huiSell`；图表上这类日期直接没有数据点即可（不是断线，因为源站本来就没提供）。真正的"空档"只针对源站窗口内存在、但本地 IndexedDB 缺失的日期。
